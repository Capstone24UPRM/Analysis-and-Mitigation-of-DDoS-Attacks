    ...
    const pollAttackStatus = setInterval(async () => {
      try {
        ...
        if (data.status !== "bad") {
          setAttackLogs((prev) => [
            ...prev,
            addTimestamp(`Attack Status: ${data.status}`),
          ]);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const pollMLStatus = setInterval(async () => {
      try {
        ...
        setMlLogs((prev) => [
          ...prev,
          addTimestamp(`ML Status: ${data.status}`),
        ]);
      } catch (error) {
    ...
    const pollWebsiteStatus = setInterval(async () => {
      try {
        ...
        if (data.status !== "bad") {
          setWebsiteLogs((prev) => [
            ...prev,
            addTimestamp(`Website Status: ${data.status}`),
          ]);
        }
      } catch (error) {
...
>>>>>>> 187751fcea91ed50d6faea00e48c2c5f13735698
      </div>
    </div>
  );
}
